...
namespace ScriptLib
{
   ...
   {
   public:
      ScriptBase() : mbLuaState(false), mbLoaded(false), sScriptName(""), m_OriginalPackagePath(""), mlState(nullptr)
      {
      };

      ScriptBase(const ScriptBase& source)  :
         mbLuaState(source.mbLuaState), mbLoaded(source.mbLoaded), sScriptName(source.sScriptName),
         m_OriginalPackagePath(source.m_OriginalPackagePath), mlState(source.mlState)
      {

      }

      ScriptBase& operator=(const ScriptBase& source)
      {
         if (this!=&source) {
            mbLuaState = source.mbLuaState;
            mbLoaded = source.mbLoaded;
            sScriptName = source.sScriptName;
            m_OriginalPackagePath = source.m_OriginalPackagePath;
            mlState = source.mlState;
         }
      ...
      void cleanup(void)
      {
         addStringGlobalToTable(mlState, "package", "path", m_OriginalPackagePath);
 ...
 */
      ...
      static std::string getStringValue(lua_State *lua, const std::string &tablename);
      static double getDoubleValue(lua_State *lua, const std::string &tablename);
      static int getNumberValue(lua_State *lua, const std::string &tablename);
   ...
   protected:
      ...
      static int catchLuaError(lua_State *L)
      {
         ...
         throw( RuntimeError( "Lua script badness occurred!")); /*,__FILE__,__LINE__ ) );*/
